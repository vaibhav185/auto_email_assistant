...
def main():
    ...
    for email_data in emails:
        ...
        # Runnable chains use .invoke() instead of .run()
        summary = summarize_chain.invoke({"email_content": content})
        priority = classify_chain.invoke({"email_content": content})
        reply = reply_chain.invoke({"email_content": content})
        review = needs_review(summary, priority, reply)
